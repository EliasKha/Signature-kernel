...
# ---------- data directory resolution (outside repo) ----------

def _data_dir() -> Path:
    env = os.environ.get("LOGSIG_RL_DATA_DIR")
    if env:
        return Path(env)
    return Path.home() / "logsig-rl-data"  # default outside the repo


def _resolve_csv(ticker: str) -> Path:
    return _data_dir() / f"market_{ticker.upper()}.csv"
...
def load_market_data(ticker: str, years: int) -> pd.DataFrame:
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"{path} not found.\n"
            f"Download outside the repo, e.g.:\n"
            f"  python -m logsig_rl.envs.download --ticker '{ticker}' --years {years} --outdir \"{_data_dir()}\""
        )
